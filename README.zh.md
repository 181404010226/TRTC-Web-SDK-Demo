...
- **必须使用内网穿透生成的URL访问应用**，而非localhost地址
- 通过内网穿透URL访问时，系统会自动填写SDKAppID和SDKSecretKey
- 邀请链接功能依赖内网穿透地址才能正常工作
- 确保在会议期间内网穿透工具和服务器始终运行
...
- 一般情况下体验 Demo 需要通过内网穿透URL访问，例如`https://xxx.ngrok.io`或花生壳提供的域名
- 目前桌面端 Chrome 浏览器支持 TRTC Web SDK 的相关特性比较完整，因此建议使用 Chrome 浏览器进行体验。

4. 开始使用

- 您会看到SDKAppID和SDKSecretKey已自动填写（绿色提示条）
- 输入 userId 和 roomId
- 点击【进入房间】按钮进入房间
- 点击【开始采集音频/视频】按钮，可采集麦克风或摄像头
- 点击【停止采集音频/视频】按钮，可终止采集麦克风或摄像头
- 点击【开始共享屏幕】按钮开始屏幕分享
- 点击【停止共享屏幕】按钮取消屏幕分享
...
>! 重要提醒
> - 确保使用内网穿透URL访问应用，否则自动配置功能将失效
> - 正式环境中应将UserSig生成代码部署在服务器上，避免密钥泄露
